...
+(void)setRotation//zl
{
    ...
    if (orientation == UIInterfaceOrientationPortrait) {
        ...
        [self sharedView].transform=CGAffineTransformMakeRotation(-M_PI_2);
    } else if (orientation == UIInterfaceOrientationLandscapeRight){
        
        [self sharedView].transform= CGAffineTransformMakeRotation(M_PI_2);
    } else if (orientation == UIInterfaceOrientationPortraitUpsideDown) {
        
        [self sharedView].transform= CGAffineTransformMakeRotation(M_PI);
    }
//    [self sharedView].transform=CGAffineTransformMakeRotation(M_PI_2);//zl
}
+(void)handleRotation
{
    [self sharedView].transform=CGAffineTransformMakeRotation(M_PI_2);
}
+(void)setRevertRotation//zl
{
...
+ (void)showImage:(UIImage*)image status:(NSString*)status {
    NSTimeInterval displayInterval =2.5; //[self displayDurationForString:status];//zl
