...
- (UILabel *)describeLabel {
    if (!_describeLabel) {
        ...
        _describeLabel.textAlignment =NSTextAlignmentNatural;//NSTextAlignmentLeft;// NSTextAlignmentCenter;
...
- (void)setDescribeArray:(NSArray *)describeArray{
    ...
    if (!describeArray.count) {
        ...
        _describeLabel.text = [NSString stringWithFormat:@"      %@",_describeArray[_currIndex]] ;
    }
    //重新计算pageControl的位置
...
- (void)setPagePosition:(PageControlPosition)pagePosition {
    ...
    else if (_pagePosition==PositionCustom)
        _pageControl.frame = CGRectMake(self.width - HORMARGIN - size.width, self.describeLabel.centerY, size.width, size.height);
...
- (void)changeToNext {
    ...
    self.describeLabel.text =[NSString stringWithFormat:@"      %@",self.describeArray[self.currIndex]] ;
}
